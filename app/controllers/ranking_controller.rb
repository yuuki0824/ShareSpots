class RankingController < ApplicationController
  ...
  def like_ranking
    @title = "like ranking"
    @rank = Like.group(:spot_id).order('count_spot_id desc').limit(5).count('spot_id')
    spot_ids = @rank.keys
  ...
  def follower_ranking
    ...
    @rank = Relationship.group(:followed_id).order('count_followed_id desc').limit(5).count('followed_id')
